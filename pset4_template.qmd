...
tx_zip_hospitals.plot(column='hospital_count', legend=True)

```


```{python}
tx_zip_hospitals.sort_values(by="hospital_count", ascending=False)

# Check if any zip codes in texas_summary_2016 match those in tx_zip_codes
matching_zip_codes = set(texas_summary_2016['zip_code']).intersection(set(tx_zip_codes['ZCTA5']))
print("Number of matching zip codes:", len(matching_zip_codes))
