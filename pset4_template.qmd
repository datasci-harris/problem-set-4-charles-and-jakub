...
zip_closures_geo = zip_closures_geo.set_geometry('geometry_x') 

tx_zip_codes = tx_zip_codes.to_crs(epsg=3857)
zip_closures = zip_closures_geo.to_crs(epsg=3857)
...
#Spatial join
spatial_join = gpd.sjoin(tx_zip_codes, buffer_zone, how="inner", predicate="intersects")

```

```{python}

num_indirectly_affected = spatial_join['ZCTA5_x'].nunique()
...
#TEST CODE ONLY, DO NOT INCLUDE IN FINAL

import matplotlib.pyplot as plt

# Create a plot to visualize the buffer zones and zip codes
fig, ax = plt.subplots(figsize=(10, 10))

# Plot all Texas zip codes in light gray for context
tx_zip_codes.plot(ax=ax, color="lightgrey", edgecolor="white", label="All Texas Zip Codes")

# Plot the buffer zones around directly affected zip codes in red with some transparency
buffer_zone.plot(ax=ax, color="red", alpha=0.5, edgecolor="black", label="10-Mile Buffer Zone (Directly Affected)")

# Optionally, plot the directly affected zip codes themselves
zip_closures_geo.plot(ax=ax, color="blue", marker='o', label="Directly Affected Zip Codes")

# Add a legend and title
plt.legend()
plt.title("Directly and Indirectly Affected Zip Codes in Texas (10-Mile Buffer Zone)")
...
```{python}
#More testing
# Check buffer_zone geometries and ZCTA5 values
print(buffer_zone[['ZCTA5', 'buffered_geometry']].head())
# Check intersection counts
intersection_counts = gpd.sjoin(tx_zip_codes, buffer_zone, how="inner", predicate="intersects").shape[0]
print(f"Total intersections found: {intersection_counts}")
print(tx_zip_codes.crs, buffer_zone.crs)
# Check for unique ZCTA5 values in tx_zip_codes and buffer_zone
print(tx_zip_codes['ZCTA5'].unique())
print(buffer_zone['ZCTA5'].unique())
...
indirectly_affected_zips = buffer_zone['ZCTA5'].unique()
tx_zip_codes.loc[
    (tx_zip_codes['ZCTA5'].isin(indirectly_affected_zips)) &
    (~tx_zip_codes['ZCTA5'].isin(directly_affected_zips)) 
, 'status'] = 'Indirectly Affected'
print("Indirectly affected zips:", indirectly_affected_zips)
...
```{python}
indirectly_affected_filtered = tx_zip_codes[
    (tx_zip_codes['ZCTA5'].isin(indirectly_affected_zips)) &
    (~tx_zip_codes['ZCTA5'].isin(directly_affected_zips))
]
print("Filtered indirectly affected zip codes:", indirectly_affected_filtered['ZCTA5'].unique())
